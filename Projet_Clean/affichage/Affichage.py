...
class Affichage(threading.Thread):
    ...
    def dessiner_trace(self):
        """ Q1.2 : Dessiner la trajectoire du robot en bleu. """
        #q1.3
        if not self.robot.crayon_abaisse:
            return
        
        if len(self.robot.historique_trace) < 2:
            return
        ...
        ''' points_ecran = [
            ...
            for x, y in self.robot.historique_trace

        ]



        # Dessiner les lignes en bleu

        pygame.draw.lines(self.screen, (0, 0, 255), False, points_ecran, 2)
        '''

        with self.lock:
            for i in range(1, len(self.robot.historique_trace)):
                p1 = self.robot.historique_trace[i-1] # p1 est (x, y, color)
                p2 = self.robot.historique_trace[i]   # p2 est (x, y, color)
                
                # Protection : on vérifie que p2 a bien la couleur (3 éléments)
                if len(p2) < 3:
                    continue 

                # Conversion physique -> pixels
                pos1 = (p1[0] * cfg.SCALE, p1[1] * cfg.SCALE)
                pos2 = (p2[0] * cfg.SCALE, p2[1] * cfg.SCALE)
                
                # On dessine segment par segment avec la couleur enregistrée
                pygame.draw.line(self.screen, p2[2], pos1, pos2, 2)
