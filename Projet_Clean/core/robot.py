...
class Robot:
    ...
    def __init__(self, rayon_roue_m: float, ecartement_roues_m: float, ori_initiale="droite", x=0.0, y=0.0):
        ...
        # Q1.2 : Variables pour le crayon
        self.historique_trace = []  
        self.crayon_abaisse = True  
        self.couleur_active = (0, 0, 255) # Couleur par défaut: bleu pour q1.4

    def change_couleur(self, couleur):
        """ q1.4 Modifie la couleur utilisée pour les prochains segments. """
    ...
    def step(self, monde):
        ...
        # Q1.2/1.4 : Optimisation pour ne pas surcharger la mémoire
        
            ajouter = False
            if not self.historique_trace:
                ajouter = True
            else:
                derniere_pos = self.historique_trace[-1]
            # Calcul de la distance avec la dernière position enregistrée
                dist = math.hypot(self.pos.x - derniere_pos[0], self.pos.y - derniere_pos[1])
                if dist > 0.02: # 2 cm
                    ajouter = True
        
            if ajouter:
                self.historique_trace.append((self.pos.x, self.pos.y, self.couleur_active))
      
    
    #pour Q1.3
    def dessine(self, b: bool):
        self.crayon_abaisse = b