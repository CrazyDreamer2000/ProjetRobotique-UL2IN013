...
class Affichage(threading.Thread):
    ...
    def __init__(self, robots, monde, lock):
        super().__init__()
        self.robots = robots # maintenant une liste
        self.monde = monde
        self.running = True
        self.fps = 60
    ...
    def dessiner_robot(self):
        for robot in self.robots:
            ...
            points_monde = [
                ...
                for x, y in transformer_polygone_local_vers_monde(self.monde.poly_robot_local, robot.pos)
                ]

                # Centre du robot + orientation actuelle
            cx, cy = robot.pos.x * cfg.SCALE, robot.pos.y * cfg.SCALE
            ori = robot.pos.orientation

            # Ligne du robots
            self.trace.append(((cx, cy),robot.couleur_trace))
            if robot.dessiner:
                for point in self.trace:
                    pygame.draw.line(self.screen, point[1], point[0], point[0])
